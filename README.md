...
```bash
dk bash  # starts bash inside "slackclone" container
...
* `dkup` uses docker-compose to start 3 containers: postgres, nginx, and slackclone.
...
* Once `dkup` is running, `dk <command>` will run `<command>` inside the `slackclone` container. So `dk bash` will get you "logged in" as root inside that container. Once inside, you need to run Django's `manage.py` commands to initialize the database properly.
* The slackclone container runs 3 services:
 * django on port 8000
...
```bash
mkvirtualenv slackclone -p python3  # creates a python3 virtualenv
...
Install docker and nginx. Create an empty postgres database slackclone owned by a user slackclone.

On your remote machine, create a file ~/slackclone.env:

```
DJANGO_DB_PASSWORD=<slackclone's password>
DJANGO_DB_HOST=<database_ip>
DJANGO_DB_NAME=slackclone
DJANGO_DB_USER=slackclone
DJANGO_DEBUG=0
```
...
```
server {
    server_name  slackclone.example.com;
    ...
    location /static {
        alias /home/ubuntu/dkdata/slackclone/static;
...
(Replace "slackclone.example.com" with your production domain)
