...
GREEN='\033[00;32m'
YELLOW='\e[0;33m'
HOST_PROD=slackclone.example.com
...
function dknpminstall {
    CD=$(pwd)
    cd $PROJ_BASE
    docker run -it --rm -v $(pwd):/app -w /app/frontend -e NODE_ENV=development slackclone npm install
    exitcode=$?
    cd $CD
    return $exitcode
}
...
function dkrun_prod {
    docker stop slackclone
    docker rm slackclone
    docker run --name slackclone -d --env-file /home/ubuntu/slackclone.env \
        -p 3000:3000 -p 8000:8000 \
        -v /home/ubuntu/dkdata/slackclone:/dkdata \
        slackclone start_web.sh
}

function deploy_prod {
  rsync -av --exclude frontend/node_modules --exclude frontend/.nuxt * ubuntu@$HOST_PROD:./slackclone/
...
echo_green "Welcome to slackclone's dev env"
